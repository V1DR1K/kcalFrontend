...
export function TrainingCalendar({ api }) {
  ...
  const load = useCallback(async () => { const [days, exercises] = await Promise.all([trainingApi.calendar(api, from, to), trainingApi.exercises(api)]); const ids = [...new Set(days.flatMap((day) => (day.plannedPlans || []).map((item) => item.planId)).filter(Boolean))]; const details = await Promise.all(ids.map(async (id) => { try { return await trainingApi.plan(api, id); } catch { return null; } })); return { days, planDetails: details.filter(Boolean), exercises: exercises.items || [] }; }, [api, from, to]);
