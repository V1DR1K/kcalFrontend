...
import { dateKey, formatDuration, monthDays, normalizeSession, sessionsForCalendar } from "./training-utils";
...
function TrainingDayDetail({ api, selected, planDetails, onClose, onEdit, onStart, onDeleted }) {
  ...
  return <ModalShell title={visible?.title || visible?.planName || fullDate(selected.date)} description={visible ? fullDate(visible.date) : "No registraste una sesión este día."} onClose={onClose} theme="training" className="training-session-detail" backdropClassName="training-session-backdrop" footer={visible ? <><button type="button" className="training-danger-button" onClick={remove}><Icon name="delete" />Eliminar</button><button type="button" className="training-primary" onClick={() => onEdit(visible)}><Icon name="edit" />Editar</button></> : <><button type="button" className="training-secondary" onClick={onClose}>Cerrar</button><button type="button" className="training-primary" onClick={() => onStart(firstPlan || { module: "GYM" }, selected.date)}><Icon name="add" />Registrar</button></>}>
    <div className="training-session-detail-content" data-dialog-scroll-owner="true">{selected.plannedPlans.length > 0 && <section className="training-day-planned"><h3>Planificado</h3>{selected.plannedPlans.map((schedule) => { const plan = planDetails.get(String(schedule.planId)); const day = plan?.days?.find((item) => String(item.id) === String(schedule.planDayId)); return <div className="training-day-plan-row" key={`${schedule.planId}-${schedule.planDayId}`}><div><TrainingModuleBadge module={schedule.module} /><strong>{day?.name || schedule.planDayName}</strong><small>{plan?.name || "Plan"} · {plan?.frequencyMode === "DYNAMIC" ? "siguiente dinámico" : "día fijo"}</small></div><button type="button" className="training-secondary" disabled={!day} onClick={() => onStart(schedule, selected.date)}><Icon name="play_arrow" />Iniciar</button></div>; })}</section>}{selected.sessions.length > 1 && <section className="training-day-performed"><h3>Sesiones registradas</h3>{selected.sessions.map((item) => <div key={item.id} className="training-day-session-row"><TrainingModuleBadge module={item.type} /><strong>{item.title || "Sesión sin título"}</strong><span>{item.status || "Registrada"} · {formatDuration(item.durationMinutes)}</span></div>)}</section>}{loading && <TrainingStatus loading />}{error && <TrainingStatus error={error} />}{!loading && !error && visible && <><div className="training-detail-summary"><TrainingModuleBadge module={visible.type} /><span>{formatDuration(visible.durationMinutes)}</span><span>{visible.exercises.length} ejercicios</span></div><div className="training-detail-exercises">{visible.exercises.length ? visible.exercises.map((exercise) => <article key={exercise.id || exercise.name}><div><strong>{exercise.name}</strong>{exercise.notes && <small>{exercise.notes}</small>}</div><span>{exercise.sets.map((set) => `${set.reps || 0} reps${visible.type === "GYM" ? ` · ${set.weightKg || 0} kg` : ""}`).join(" · ")}</span></article>) : <p className="training-detail-note">La sesión no tiene ejercicios detallados disponibles.</p>}</div>{visible.notes && <p className="training-detail-note">{visible.notes}</p>}</>}{!loading && !error && !visible && selected.plannedPlans.length === 0 && <div className="training-empty-inline"><Icon name="today" /><span>Elegí Registrar para anotar una sesión en esta fecha.</span></div>}</div>
  </ModalShell>;
}
...
export function TrainingCalendar({ api }) {
  ...
  const load = useCallback(async () => {
    const days = await trainingApi.calendar(api, from, to);
    const ids = [...new Set(days.flatMap((day) => (day.plannedPlans || []).map((item) => item.planId)).filter(Boolean))];
    const details = await Promise.all(ids.map(async (id) => { try { return await trainingApi.plan(api, id); } catch { return null; } }));
    const exercises = await trainingApi.exercises(api);
    return { days, planDetails: details.filter(Boolean), exercises: exercises.items || [] };
  }, [api, from, to]);
  const resource = useTrainingData(load, [load]);
  const days = Array.isArray(resource.data?.days) ? resource.data.days : [];
  const sessions = useMemo(() => sessionsForCalendar(days), [days]);
  const planDetails = useMemo(() => new Map((resource.data?.planDetails || []).map((plan) => [String(plan.id), plan])), [resource.data]);
  const byDate = useMemo(() => days.reduce((map, day) => { map.set(day.date, { sessions: sessions.filter((session) => session.date === day.date), plannedPlans: day.plannedPlans || [] }); return map; }, new Map()), [days, sessions]);
