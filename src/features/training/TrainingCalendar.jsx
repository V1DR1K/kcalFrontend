...
function TrainingSessionDetail({ api, session, date, onClose, onEdit, onStart, onDeleted }) {
  ...
  return <ModalShell title={visible ? visible.routineName : fullDate(date)} description={visible ? fullDate(visible.date) : "No registraste una sesión este día."} onClose={onClose} className="training-session-detail" backdropClassName="training-session-backdrop" footer={visible ? <><button type="button" className="training-danger-button" onClick={remove}><Icon name="delete" />Eliminar</button><button type="button" className="training-primary" onClick={() => onEdit(visible)}><Icon name="edit" />Editar</button></> : <><button type="button" className="training-secondary" onClick={onClose}>Cerrar</button><button type="button" className="training-primary" onClick={() => onStart("GYM", date)}><Icon name="add" />Registrar</button></>}>
