...
export function CardioPage({ api }) {
  ...
  const load = useCallback(async () => {
    ...
    const [records, summary, week] = await Promise.all([trainingApi.cardio(api, { page }), trainingApi.cardioSummary(api), trainingApi.cardioWeekly(api, { date: dateKey, timeZone: localTimeZone() })]);
    return { records: records.items || [], page: records, summary, week, today: dateKey };
  }, [api, page]);
  ...
  return <section className="page training-page training-cardio-page"><Header title="Cardio" action={<button type="button" className="training-primary" onClick={() => setEditor({})}><Icon name="add" />Registrar sesión</button>} /><p className="training-page-intro">Registrá tu caminadora y consultá tus pasos estimados.</p><TrainingStatus loading={resource.loading} error={resource.error} onRetry={resource.reload} />{!resource.loading && !resource.error && <>
    ...
    <section className={`training-surface training-cardio-service-card training-cardio-maintenance ${due ? "is-due" : ""}`.trim()}><div className="training-cardio-service-heading"><div><h2>Mantenimiento de la caminadora</h2><p>{summary.latestService ? `Último service: ${formatCardioDate(summary.latestService.servicedAt)}` : "Todavía no registraste un service."}</p></div><div className="training-cardio-service-icon"><Icon name={due ? "warning" : "build"} /></div></div><div className="training-cardio-kpi-grid"><div className="training-cardio-kpi-primary"><span>Uso desde el último service</span><strong>{formatCardioMinutes(summary.totalDurationMinutes)}</strong><small>de {formatCardioMinutes(summary.thresholdMinutes)} hasta el próximo service</small></div><div className="training-cardio-kpi-secondary"><span>{due ? "Service recomendado" : "Tiempo restante"}</span><strong>{due ? "Ahora" : formatCardioMinutes(summary.remainingMinutes)}</strong><small>{due ? "Ya alcanzaste el límite de 20 horas" : "La cuenta se actualiza con cada sesión"}</small></div></div><div className="training-cardio-progress" role="progressbar" aria-label="Horas de uso desde el último service" aria-valuenow={Math.min(Number(summary.totalDurationMinutes || 0), Number(summary.thresholdMinutes || 1200))} aria-valuetext={`${formatCardioMinutes(summary.totalDurationMinutes)} de ${formatCardioMinutes(summary.thresholdMinutes)}`} aria-valuemin="0" aria-valuemax={Number(summary.thresholdMinutes || 1200)}><span style={{ width: `${progress}%` }} /></div><div className="training-cardio-service-footer"><span>{summary.latestService ? "El contador toma las sesiones posteriores a este service." : "El contador suma tus sesiones hasta que cargues el primer service."}</span><button type="button" className={due ? "training-primary" : "training-secondary"} onClick={() => setServiceEditorOpen(true)}><Icon name="build" />Registrar service</button></div></section>

  </>}{editor && <CardioRecordEditor api={api} record={editor.id ? editor : null} heightCm={summary.profileHeightCm} onClose={() => setEditor(null)} onSaved={resource.reload} />}{serviceEditorOpen && <CardioServiceEditor api={api} onClose={() => setServiceEditorOpen(false)} onSaved={resource.reload} />}</section>;
}
