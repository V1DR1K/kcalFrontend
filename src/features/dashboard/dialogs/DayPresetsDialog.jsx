...
import { formatMealLogAmount, mealLogName, mealTotals } from "../dashboard.utils";
...
export function DayPresetsDialog({ api, user, date, data, mealTypes, presets, onReload, onApplied, open, onOpen, onClose, FoodPickerComponent }) {
  ...
  async function saveEditor() {
    ...
    setSaving(true); setError("");
    try { const updated = await api.request(`/api/nutrition/day-presets/${editor.id}`, { method: "PUT", body: JSON.stringify({ name: editor.name.trim(), items: editor.items.map((item) => ({ ...item, quantity: decimalNumber(item.quantity) })) }) }); await onReload(); setEditor(updated); api.notify("Preset actualizado."); }
  ...
  return <>
    ...
    >
        ...
        {editor ? <div className="day-preset-editor-body">
          ...
          <div className="day-preset-items" data-dialog-scroll-owner="true">{editor.items.map((item, index) => <article className="day-preset-item" key={`${item.itemType}:${item.itemId || item.displayName}:${index}`}>
            <div className="day-preset-item-title"><strong>{item.displayName || (item.itemType === "RECIPE" ? "Receta" : item.itemType === "AI_ESTIMATE" ? "Estimación" : "Alimento")}</strong><button type="button" className="icon-button danger-text" aria-label="Quitar elemento" onClick={() => setEditor((current) => ({ ...current, items: current.items.filter((_, itemIndex) => itemIndex !== index) }))}><Icon name="delete" /></button></div>
             <div className="day-preset-item-fields"><label><span>Comida</span><select value={item.mealType} onChange={(event) => setEditor((current) => ({ ...current, items: current.items.map((entry, itemIndex) => itemIndex === index ? { ...entry, mealType: event.target.value } : entry) }))}>{mealTypes.map((meal) => <option key={meal.code} value={meal.code}>{meal.label}</option>)}</select></label><label><span>{item.itemType === "RECIPE" && item.unit === "GRAM" ? "Gramos cocidos" : "Cantidad"}</span><input type="text" inputMode="decimal" value={item.quantity} aria-label={item.itemType === "RECIPE" ? formatMealLogAmount(item) : "Cantidad"} onChange={(event) => setEditor((current) => ({ ...current, items: current.items.map((entry, itemIndex) => itemIndex === index ? { ...entry, quantity: normalizeDecimalInput(event.target.value) } : entry) }))} /></label><label><span>Unidad</span><select value={item.unit} onChange={(event) => setEditor((current) => ({ ...current, items: current.items.map((entry, itemIndex) => itemIndex === index ? { ...entry, unit: event.target.value } : entry) }))}><option value="GRAM">{item.itemType === "RECIPE" ? "Gramos cocidos" : "Gramos"}</option><option value="PORTION">Porciones</option><option value="UNIT">Unidades</option><option value="MILLILITER">Mililitros</option></select></label></div>
          </article>)}</div>
