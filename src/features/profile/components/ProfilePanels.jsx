...
export function ChangePasswordForm({ api }) {
  ...
  async function submit(event) { event.preventDefault(); setFieldErrors({}); if (newPassword.length < 8) return setFieldErrors({ newPassword: "La contraseña debe tener al menos 8 caracteres." }); if (newPassword !== confirmPassword) return setFieldErrors({ confirmPassword: "Las contraseñas no coinciden." }); if (saving) return; setSaving(true); try { const session = await api.runAction({ title: "Actualizando contraseña", description: "Creamos una nueva sesión segura..." }, () => api.request("/api/auth/change-password", { method: "PUT", body: JSON.stringify({ currentPassword, newPassword }) }), { quiet: true }); if (session?.token) localStorage.setItem(TOKEN_KEY, session.token); if (session?.refreshToken) localStorage.setItem(REFRESH_KEY, session.refreshToken); setCurrentPassword(""); setNewPassword(""); setConfirmPassword(""); api.notify("Contraseña actualizada. Tu sesión sigue activa."); } catch (error) { api.notify(error?.message || "No se pudo actualizar la contraseña.", "error"); } finally { setSaving(false); } }
...
export function MacroControl({ label, value, grams, onChange, tone }) { return <label className={`macro-control ${tone}`}><span><strong>{label}</strong><small>{grams}g</small></span><output>{formatNumber(value, 1)}%</output><input type="range" min="0" max="100" step="0.5" value={value} onChange={(event) => onChange(event.target.value)} /></label>; }
