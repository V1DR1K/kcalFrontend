...
export function DayPresetsPage({ api, user, seed, onSeedConsumed }) {
  ...
  useEffect(() => {
    ...
    Promise.all(itemsToHydrate.map(async (item) => {
      const key = presetItemCacheKey(item);
      if (presetItemCache.current.has(key)) return [key, await presetItemCache.current.get(key)];
      const itemType = item.itemType || item.type;
      const endpoint = itemType === "RECIPE" ? `/api/recipes/${item.itemId}` : `/api/foods/${item.itemId}`;
      const request = api.request(endpoint, { cache: "no-store" }).then((detail) => detail || null).catch(() => null);
      presetItemCache.current.set(key, request);
