...
function PresetEditorDialog({ api, user, editor, mealTypes, onClose, onSaved }) {
  ...
  return <ModalShell as="form" onClose={onClose} closeDisabled={saving} title={isNew ? "Guardar día" : "Editar día"} description="Definí el nombre, la descripción y las comidas que querés repetir." className="abm-editor-modal day-preset-editor-modal" backdropClassName="abm-editor-backdrop" wrapContent={false} dialogProps={{ onSubmit: save }} footer={<><button type="button" className="secondary" disabled={saving} onClick={onClose}>Cancelar</button><button type="submit" className="primary" disabled={saving}>{saving ? "Guardando…" : isNew ? "Guardar día" : "Guardar cambios"}</button></>}>
    <div className="abm-editor-body" data-dialog-scroll-owner="true">
      ...
      <div className="day-editor-items" aria-label="Comidas del día">
        {draft.items.map((item, index) => <article className="day-editor-item" key={`${item.itemId || item.displayName}-${index}`}>
          <div className="day-editor-item-main"><div className="day-editor-thumb"><img src={item.imageUrl || "/category-assets/other.webp"} alt="" onError={(event) => { event.currentTarget.onerror = null; event.currentTarget.src = "/category-assets/other.webp"; }} /></div><div><strong>{item.displayName || "Alimento"}</strong><small>{mealLabel(item.mealType)} · {formatNumber(item.calories)} kcal</small></div></div>
          <div className="day-editor-controls"><label><span>Comida</span><select value={item.mealType} onChange={(event) => updateItem(index, "mealType", event.target.value)}>{mealTypes.map((meal) => <option key={meal.code} value={meal.code}>{meal.label}</option>)}</select></label><label><span>Cantidad</span><input inputMode="decimal" value={item.quantity} onChange={(event) => updateItem(index, "quantity", normalizeDecimalInput(event.target.value))} /></label><label><span>Unidad</span><select value={item.unit} onChange={(event) => updateItem(index, "unit", event.target.value)}><option value="GRAM">Gramos</option><option value="MILLILITER">Mililitros</option><option value="UNIT">Unidades</option><option value="PORTION">Porciones</option></select></label><button type="button" className="icon-button danger-text" aria-label={`Quitar ${item.displayName || "elemento"}`} onClick={() => setDraft((current) => ({ ...current, items: current.items.filter((_, itemIndex) => itemIndex !== index) }))}><Icon name="delete" /></button></div>
        </article>)}
      </div>
