...
export function Shell({ children, page, mode, setPage, setMode, logout }) {
  ...
  return <div className={`app-shell ${training ? "training-shell" : ""}`.trim()} data-app-mode={mode} data-nav-mode={mode}><aside className="sidebar"><div className="brand"><Icon name="scale" className="fill" /><div><strong>{APP_NAME}</strong><span>{training ? "Entrenamiento" : "Bitácora diaria"}</span></div></div><nav aria-label={training ? "Navegación de entrenamiento" : "Navegación principal"}><ModeNavigation activeMode={mode} page={page} setPage={setPage} setMode={setMode} /></nav><button className="ghost" onClick={logout}><Icon name="logout" />Salir</button></aside><main className="content" data-app-scroll-root="true" key={`${mode}-${page}`}>{children}</main><nav className={`mobile-nav ${training ? "training-mobile-nav" : ""}`.trim()} aria-label={training ? "Navegación de entrenamiento" : "Navegación principal"}><MobileNavigation key={mode} mode={mode} page={page} setPage={setPage} setMode={setMode} /></nav></div>;
}
